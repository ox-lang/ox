...
```Clojure
(in-ns 'scratch)                       ;; special form
	                                   ;; creates a new empty environment
									   ;; sets the value of *ns* to the new env

(require '[clojure/core :refer :all])  ;; loads in the clojure core & creates aliases
                                       ;; into the value of *ns* by mutating the env value

(def foo 3)                            ;; creates the var #'scratch/foo
                                       ;; adds it to the current namespace (mutation)
...
In Clojure, the reader will construct complex objects such as maps and
UUIDs at read time before the read code is evaluated by `eval`,
`macroexpand` or `compile`. To quote Paul Philips, "the minimum test
of a parser is that it parses". The parser should map 1:1 from the
textual representation of a program to a tree structure. This tree
structure can then be munged, but reading a tree in shouldn't involve
doing anything to it. Thus Oxlang divides what is traditionally `read`
in most Lisps into `parse` and `read-eval`. `parse` as the name
implies, is a pure function of input text which returns a tree
representing the read structures or fails. See
[the parser docs](/doc/parser.md) for more information on the
specifics of the parser and the language.

`read-eval` is a function of an environment and a tree, which walks
the parsed tree of primitive types generated by `parse` constructing a
new tree which may contain arbitrary values. This makes clear the data
dependency between "reading" and the reading environment which Clojure
sweeps under the rug with it's composed read and evaluate
behavior. The `read-eval` is a leaf-then-parent macro-expansion pass
where the only forms altered are `(read-eval EXPR)`. These forms are
replaced with the result of evaluating `EXPR` in the environment
passed to `read-eval` as an argument.

For example, the form `{:foo :bar}` reads to the rather nasty structure

```Clojure
(read-eval (hash-map (((read-eval (keyword "foo")) (read-eval (keyword "bar"))))))
```
...
```Clojure
> (read-eval (hash-map (((read-eval (keyword "foo")) (read-eval (keyword "bar"))))))
> (read-eval (hash-map ((:foo (read-eval (keyword "bar"))))))
> (read-eval (hash-map ((:foo :bar))))
> {:foo :bar}
```

This is considered a feature as it decomplects the implementation of
the keyword, hash map and other standard library structures from the
Ox language, meaning that keywords and hashmaps could be implemented
in ox and defined as part of bootstrapping. Users wishing to make use
of the `{}` notation for instance must import a namespace providing
the appropriate constructor `hash-map`. While potentially inefficient,
this means that porting Ox between platforms and targets is simplified
since only a few datastructures are actually needed and a reference
implementation in terms of these is provided. Once ported, Ox can be
specialized for performance by using host specific datastructure
implementations at the cost of some complexity however the abstraction
and resulting flexibility remains.

## Macros

Macros are ultimately a compiler extension tool. Macros enable
